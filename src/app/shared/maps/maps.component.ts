...
export class MapsComponent {
  ...
  public mapTypes = [
    {
      id: 'satellite',
      name: 'Satellite',
      svg: 'assets/images/satellite.svg',
      tooltip: 'Switch to Satellite',
    },
    {
      id: 'terrain',
      name: 'Terrain',
      svg: 'assets/images/terrain.svg',
      tooltip: 'Switch to Terrain',
    },
  ];
    ...
    try {
      google.maps.event.addListener(marker, 'mouseover', (e: any) => {
        ...
        if (!data?.tooltip?.length) {
          ...
          const colLength: any = tooltipOpt?.length
            ? tooltipOpt?.length < 2
              ? 12
              : tooltipOpt?.length < 3
              ? 6
          ...
          for (const eachLabelVal of data?.tooltip) {
            htmlData =
              htmlData +
              (tooltipOpt?.length && tooltipOpt?.includes(eachLabelVal?.key)
                ? `<div class="col-${colLength} pl-2 pr-0 m-0 mt-1">
          <span class="span-minified">${eachLabelVal['label']}</span>
          <p class="inter-medium mb-0" style="color: ${
            eachLabelVal?.color ? eachLabelVal?.color : '#000'
          }">${eachLabelVal['value']}</p>
          </div>`
                : '');
          }
          if (htmlData) {
  ...
  openTooltip(tooltipObj: any) {
    ...
    const colLength: any = tooltipObj['tooltipOpt']?.length
      ? tooltipObj['tooltipOpt']?.length < 2
        ? 12
        : tooltipObj['tooltipOpt']?.length < 3
        ? 6
        : tooltipObj['tooltipOpt']?.length < 4
        ? 4
        : tooltipObj['tooltipOpt']?.length < 13
        ? 3
        : 2
      : 0;
    for (const eachLabelVal of tooltipObj?.eachMarker?.tooltip) {
      htmlData =
        htmlData +
        (tooltipObj['tooltipOpt']?.length &&
        tooltipObj['tooltipOpt']?.includes(eachLabelVal?.key)
          ? `<div class="col-${colLength} pl-2 pr-0 m-0 mt-1">
        <span class="span-minified">${eachLabelVal['label']}</span>
        <p class="inter-medium mb-0" style="color: ${
          eachLabelVal?.color ? eachLabelVal?.color : '#000'
